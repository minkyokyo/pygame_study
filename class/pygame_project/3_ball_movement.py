...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_LEFT: #캐릭터 왼쪽 이동
                ...
                weapon_x_pos = character_x_pos + (character_width/2) - (weapon_width/2) #캐릭터의 중간위치 - 무기 절반만큼 
    ...
    # 무기 위치 조정
    #무기 위치를 하나씩 올리는 것.
    weapons = [ [w[0],w[1]-weapon_spedd]for w in weapons]
    
    #천장에 닿은 무기 없애기
    weapons = [ [ w[0],w[1]] for w in weapons if w[1] > 0]
    
    #공 위치 정의
    # enumerate를 쓰면 balls에 있는 것들을 하나씩 가져와서 idx넣고 값을 출력해준다.
    for ball_idx, ball_val in enumerate(balls): 
        ...
        if ball_pos_y >= screen_height - stage_height - ball_height:
            ...
            ball_val["to_y"]+=0.5 #점점 빨리 바닥으로 내려오는 효과를 가진다.
